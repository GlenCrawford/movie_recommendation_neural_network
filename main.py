...
# Extract and normalize embeddings "so that the dot product between two embeddings becomes the cosine similarity".
...
# With cosine similarity, higher numbers indicate entities that are closer together, with -1 the furthest apart and +1 closest together, so all we have to do to find the closest entities in the embedding space is sort by the dot products.
def find_similar_entities(name, weights, number_of_results = 10, return_distances = False, plot = False):
  index = training_data_movie_titles_index

  try:
    entity_distances = np.dot(weights, weights[index[name]])
  except KeyError:
  ...
  if plot:
    plot_similar_entities(name, index, number_of_results, entity_distances, sorted_entity_distances)
    return None
  ...
  print(f'\n\nMovies closest to {name}.\n')

  # Print formatting.
  max_width = max([len(reverse_index_lookup(index, entity)) for entity in closest_entities])

  # Print the most similar entitites and their distances.
  for entity in reversed(closest_entities):
    print(f'Movie: {reverse_index_lookup(index, entity):{max_width + 2}} Similarity: {entity_distances[entity]:.{2}}')

def plot_similar_entities(name, index, number_of_results, entity_distances, sorted_entity_distances):
  ...
  entities_to_plot = [reverse_index_lookup(index, entity) for entity in furthest_entities]
  entities_to_plot.extend(reverse_index_lookup(index, entity) for entity in closest_entities)
...
training_data_batch_generator = generate_batch(number_of_positive_examples = 1024, negative_positive_sample_ratio = 2.0)

### Define the model ###

# Inputs (both are one-dimensional).
...
find_similar_entities('Blade Runner', movie_weights, plot = False)
